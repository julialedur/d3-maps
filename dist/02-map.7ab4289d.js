...
var coordinateStore = d3.map(); // like a d3 dictionary that stores data
...
function ready(_ref) {
  ...
  projection.fitSize([width, height], countries); // add globe

  svg.append('path').datum({
    type: 'Sphere'
  }).attr('d', path).attr('fill', '#b3d9ff').attr('stroke', 'black').attr('stroke-width', 0.5); // add countries

  svg.selectAll('.country').data(countries.features).enter().append('path').attr('class', 'country').attr('d', path).attr('fill', '#d9d9d9').attr('stroke', 'black').attr('stroke-width', 0.3); // add circles for the airports

  svg.selectAll('.airport').data(airports).enter().append('circle').attr('class', 'airport').attr('r', 2).attr('fill', 'white').attr('transform', function (d) {
    // console.log(d)
    var coords = [d.longitude, d.latitude]; // console.log(coords)
  ...
  svg.selectAll('.transit').data(flights).enter().append('path').attr('d', function (d) {
    ...
    console.log(d.coordsJFK); // Pull out our coordinates

    var fromCoords = [-73.78, 40.64]; // JFK coordinates

    var toCoords = coordinateStore.get(d.code); // Build a GeoJSON LineString
    ...
    return path(geoLine);
  }).attr('fill', 'none').attr('stroke', 'white').attr('stroke-width', 0.6).attr('opacity', 0.8).attr('stroke-linecap', 'round');
}
},{"d3":"../node_modules/d3/index.js","topojson":"../node_modules/topojson/index.js","./data/world.topojson":"data/world.topojson","./data/flights.csv":"data/flights.csv","./data/airport-codes-subset.csv":"data/airport-codes-subset.csv"}],"../node_modules/parcel/src/builtins/hmr-runtime.js":[function(require,module,exports) {
