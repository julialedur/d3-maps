...
var width = 900 - margin.left - margin.right; // add svg 

var svg = d3.select('#chart-1').append('svg').attr('height', height + margin.top + margin.bottom).attr('width', width + margin.left + margin.right).append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')'); // set projections 

var projection = d3.geoMercator();
var graticule = d3.geoGraticule();
var path = d3.geoPath().projection(projection); // scales 

var colorScale = d3.scaleSequential(d3.interpolateCool).domain([0, 600000]).clamp(true); // read in data 
...
function ready(_ref) {
  ...
  // console.log(cities)
  var countries = topojson.feature(json, json.objects.countries); // add map

  svg.selectAll('.country').data(countries.features).enter().append('path').attr('class', 'country').attr('d', path).attr('fill', 'black').attr('stroke', 'black'); // add grid 

  svg.append('path').datum(graticule()).attr('d', path).attr('stroke', 'gray').attr('fill', 'none').lower(); // add black background 

  svg.append('rect').attr('width', width).attr('height', height).attr('fill', 'black').lower(); // add cities (circles)
...
},{"d3":"../node_modules/d3/index.js","topojson":"../node_modules/topojson/index.js","./data/world.topojson":"data/world.topojson","./data/world-cities.csv":"data/world-cities.csv"}],"../node_modules/parcel/src/builtins/hmr-runtime.js":[function(require,module,exports) {
